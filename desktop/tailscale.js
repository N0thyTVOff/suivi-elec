...
/** @param {string[]} args @param {Execute} execute */
async function runTailscale(args, execute = executeFile) {
  let lastError;
  ...
  throw new Error(missing ? "Tailscale n'est pas installé." : 'Tailscale est indisponible.');
}
...
export async function tailscaleStatus(execute = executeFile) {
  try {
    ...
    const message = error instanceof Error ? error.message : 'Tailscale est indisponible.';
    return {
...
export async function enableTailscaleServe(port, execute = executeFile) {
  ...
  await runTailscale(['serve', '--bg', `http://127.0.0.1:${Number(port)}`], execute);
